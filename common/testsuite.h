...
static void mavlink_test_mpc_command(uint8_t system_id, uint8_t component_id, mavlink_message_t *last_msg)
{
#ifdef MAVLINK_STATUS_FLAG_OUT_MAVLINK1
    mavlink_status_t *status = mavlink_get_channel_status(MAVLINK_COMM_0);
        if ((status->flags & MAVLINK_STATUS_FLAG_OUT_MAVLINK1) && MAVLINK_MSG_ID_MPC_COMMAND >= 256) {
            return;
        }
#endif
    mavlink_message_t msg;
        uint8_t buffer[MAVLINK_MAX_PACKET_LEN];
        uint16_t i;
    mavlink_mpc_command_t packet_in = {
        93372036854775807ULL,963497880,101.0,129.0,157.0,185.0,213.0
    };
    mavlink_mpc_command_t packet1, packet2;
        memset(&packet1, 0, sizeof(packet1));
        packet1.time_usec = packet_in.time_usec;
        packet1.flag = packet_in.flag;
        packet1.thrust = packet_in.thrust;
        packet1.tilt_angle = packet_in.tilt_angle;
        ...
        if (status->flags & MAVLINK_STATUS_FLAG_OUT_MAVLINK1) {
           // cope with extensions
           memset(MAVLINK_MSG_ID_MPC_COMMAND_MIN_LEN + (char *)&packet1, 0, sizeof(packet1)-MAVLINK_MSG_ID_MPC_COMMAND_MIN_LEN);
        }
#endif
        memset(&packet2, 0, sizeof(packet2));
    mavlink_msg_mpc_command_encode(system_id, component_id, &msg, &packet1);
    mavlink_msg_mpc_command_decode(&msg, &packet2);
        MAVLINK_ASSERT(memcmp(&packet1, &packet2, sizeof(packet1)) == 0);

        memset(&packet2, 0, sizeof(packet2));
    mavlink_msg_mpc_command_pack(system_id, component_id, &msg , packet1.time_usec , packet1.flag , packet1.thrust , packet1.tilt_angle , packet1.torque_x , packet1.torque_y , packet1.torque_z );
    mavlink_msg_mpc_command_decode(&msg, &packet2);
        MAVLINK_ASSERT(memcmp(&packet1, &packet2, sizeof(packet1)) == 0);

        memset(&packet2, 0, sizeof(packet2));
    mavlink_msg_mpc_command_pack_chan(system_id, component_id, MAVLINK_COMM_0, &msg , packet1.time_usec , packet1.flag , packet1.thrust , packet1.tilt_angle , packet1.torque_x , packet1.torque_y , packet1.torque_z );
    mavlink_msg_mpc_command_decode(&msg, &packet2);
        MAVLINK_ASSERT(memcmp(&packet1, &packet2, sizeof(packet1)) == 0);
    ...
    mavlink_msg_mpc_command_decode(last_msg, &packet2);
        MAVLINK_ASSERT(memcmp(&packet1, &packet2, sizeof(packet1)) == 0);
        
        memset(&packet2, 0, sizeof(packet2));
    mavlink_msg_mpc_command_send(MAVLINK_COMM_1 , packet1.time_usec , packet1.flag , packet1.thrust , packet1.tilt_angle , packet1.torque_x , packet1.torque_y , packet1.torque_z );
    mavlink_msg_mpc_command_decode(last_msg, &packet2);
        MAVLINK_ASSERT(memcmp(&packet1, &packet2, sizeof(packet1)) == 0);
}
...
static void mavlink_test_common(uint8_t system_id, uint8_t component_id, mavlink_message_t *last_msg)
{
    ...
    mavlink_test_mpc_command(system_id, component_id, last_msg);
